...
export default function CopilotPage({ profile }) {
  ...
  const [pitchQuestions, setPitchQuestions] = useState([]);
  ...
  const startPitchSimulator = async () => {
    ...
    setAnswer('');
    try {
      const response = await fetch('/api/copilot/pitch-simulator/questions', { method: 'POST' });
  ...
  const handleEvaluateAnswer = async (e) => {
    ...
    try {
      const response = await fetch('/api/copilot/pitch-simulator/evaluate', {
        method: 'POST',
      ...
      if (data.success) {
        setEvaluation(data);
        setScoreHistory([...scoreHistory, data.score]);
      }
    } catch (err) {
      console.error(err);
    } finally {
                      ...
                      <div className="fade-in animate-glow" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                          <div style={{
                            ...
                            background: evaluation.score >= 8 ? 'var(--success-glow)' : evaluation.score >= 6 ? 'var(--warning-glow)' : 'var(--danger-glow)',
