...
import { Check, BookOpen, Download, AlertCircle, Sparkles, Folder } from 'lucide-react';
...
export default function Roadmap() {
  const { profile, roadmapTasks, setRoadmapTasks } = useAuth();
  ...
  const openTemplateDrawer = (guideId: string) => {
    const resource = resources.find(r => r.id === guideId);
    if (resource) {
      setSelectedTemplate(resource);
    } else {
      showToast('Template not found.', 'info');
    }
  };
          ...
          <div className="glass-card" style={{ minHeight: '400px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div className="flex-between">
              ...
              <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                Completed: {roadmapTasks.filter(t => t.completed).length} of {roadmapTasks.length}
              </span>
            </div>
